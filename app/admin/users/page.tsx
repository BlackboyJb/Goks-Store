...
import { getAllUsers, deleteUsers } from "@/lib/actions/user.actions";
...
const AdminUsersPage = async (props: {
    ...
    return <div className="space-y-2">
        ...
        <div className="overflow-x-auto">
                ...
                <TableBody>
                    {users.data.map((user) => (
                        <TableRow key={user.id}>
                            ...
                            <TableCell>
                                <Button asChild variant='outline' size='sm'><Link href={`/admin/users/${user.id}`}>Edit</Link></Button>
                                <DeleteDialogPage id={user.id} action={deleteUsers} />
                            </TableCell>
                        </TableRow>
                    ))}
                </TableBody>
            ...
            {users.totalPages > 1 && (
                <PaginationPage page={Number(page) || 1} totalPages={users?.totalPages} />
            )}
        </div>
    </div>
}
